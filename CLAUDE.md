...
### Scaling to new niches with zero human labor

When a contractor's "what do you do" answer doesn't match an existing niche, Claude normalizes it against the canonical niche list first (so "HVAC," "hvac repair," and "heating and air" all resolve to the same niche instead of fragmenting into duplicates). If it's genuinely new, Claude auto-generates the first batch of diagnostic knowledge chunks for that niche — following the exact structure the existing RAG loader already expects (`niche`, `category`, `symptom_tags`, `content`, `urgency`, `safety_flag`) — and stores it via the existing `storeKnowledgeBatch()` function. Zero human writing time per new niche, versus the old model where every niche's diagnostic knowledge was hand-written (HVAC, roofing, electrical, plumbing, landscaping, solar, water damage, tree service, lawn care, pool service, pest control — all hand-seeded in prior sessions). The same "generate dynamically per niche instead of hardcoding" principle should extend to the SMS drip/onboarding copy, which currently has HVAC-specific phrasing baked into `smsAI.js` — this needs to become templated or dynamically generated per niche rather than hardcoded.

...
- ✅ **CANCEL-by-keyword — CONFIRMED WORKING on a real phone, session 33** (previously only simulator-confirmed, session 32). Texted "Cancel" directly as the homeowner: found the right confirmed appointment, cancelled it, alerted the contractor correctly, and offered fresh rebook times in the same reply. **Still not separately verified:** whether a CANCEL-triggered rebook session respects the inter-appointment travel buffer (task #107) and the accumulated-weekday exclusion (task #105) the same way a fresh session does. Session 33's regression Test D touched the adjacent case (a second, different homeowner asking about a day that already has a booking on it) and confirmed basic double-booking avoidance, but did not specifically exercise a CANCEL-triggered rebook landing near an existing appointment — this item is still open.
- ✅ **Real-phone-only bug diagnosed AND FIXED, session 33 (Sept 11).** Root-caused as the existing 30-minute `awaiting_slot` staleness window expiring during the real-world delay between the rebook offer and the decline reply (not a phone-matching bug — ruled out via code trace). The gap — a stale reset gave zero acknowledgment that context was lost — is now fixed as part of the shared "silent reset with zero acknowledgment" fix below. Code written and `node --check` clean; not yet deployed/live-verified — see that entry for full detail and the test scripts to run after deploy.
- ✅ **Session resumption after going quiet — CONFIRMED BROKEN then FIXED, session 33 (Sept 11).** Confirmed broken via the simulator: booked an appointment, reached `awaiting_email`, backdated that session's `updated_at` by 25 hours via Railway's Postgres query tool, replied — got a brand-new session (`sessionId` changed entirely) with the generic "what's your name and address" greeting, zero reference to the existing confirmed appointment or the pending email question. **Root cause found and fixed same session, in `startHomeownerSessionInner` (`homeownerSmsAI.js`):** this function ran its "kill any stale session" UPDATE (`SET state='ended' WHERE state NOT IN ('confirmed','ended')`) *before* calling `getLastConfirmedBooking()` — but `getLastConfirmedBooking`'s own WHERE clause explicitly looks for `state IN ('confirmed','awaiting_email')`, and the UPDATE had just flipped that exact `awaiting_email` row to `'ended'` a few lines earlier in the same function call. It was checking a door that had just locked itself — a genuine ordering bug, not a missing feature, and not something that needed new infrastructure to fix (the "still at [address]?" bridge message already existed and was already wired into all three call sites in `twilio.js` — it just never got a chance to fire on this path). Fixed by reading everything needed from the about-to-expire row (both the returning-booking check AND, new, an "interrupted mid-rebook" check for a stale `awaiting_slot` session that already had a real address+service captured) *before* running the expire UPDATE, then routing whichever bridge message applies through a new `result.greeting` field that all three `twilio.js` call sites (missed-call, inbound-sms unsolicited, `/test-sms`) now check first. Both known cases — the awaiting_email resume and the CANCEL-rebook `awaiting_slot` interruption (see the bullet directly above) — are covered by this one shared fix. All deterministic templates, zero additional AI calls, zero new infrastructure or ongoing cost — the "memory" was already sitting in `appointments`/the session row itself, this just reads it before discarding it. `node --check` clean on both touched files (`homeownerSmsAI.js`, `twilio.js`). **Not yet deployed or live-verified** — needs `git push` from Jose's terminal, then run `test-session-resumption-followup.sh` (awaiting_email case, existing script) and the new `test-rebook-interruption.sh` + `test-rebook-interruption-followup.sh` (awaiting_slot interruption case) to confirm both bridge messages actually fire correctly against live Railway traffic before marking this fully closed.
...
- **Concurrent homeowners on the same contractor.** No test of two different homeowners texting the same contractor's number at the same time — the per-homeowner processing queue (task #83) was built proactively before any live stress test, but has never actually been exercised under real concurrent load. (`run-brain3-tests.sh`'s Test 5 has a basic parallel-fire scaffold for this if it hasn't been run yet — worth checking whether it was ever actually executed.)
- **Out-of-scope service request, post scope-check fixes.** E.g. texting an HVAC contractor's number about a plumbing issue — the scope classifier (task #27/#88 era) hasn't been re-verified against any of session 32/33's changes.
- **A second, independent fresh Restart → out-of-area → zip-correction → full booking run**, on a different address/contractor than the one already confirmed working, just to make sure task #102/#104's fix isn't narrowly tied to the one address it was diagnosed against.
- **The declined-weekday accumulation (#105) and inter-appointment buffer (#107) together on a busier calendar** — every test to date (including session 33's regression suite) used a contractor with a wide-open schedule; worth retesting on a contractor with a genuinely tight/partial schedule (some days closed, some days short hours) to make sure the two filters compose correctly and don't over-exclude down to zero real availability on a calendar that's actually just tighter, not empty.

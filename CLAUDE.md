...
- 🟡 **Zero-appointment natural-language query — INCONCLUSIVE, session 33 (simulator gap, not a confirmed product bug).** "Do I have any appointments with you guys?" on a fresh number with zero appointments fell through to a generic greeting in the `/api/twilio/test-sms` simulator, because the natural-language `CANCEL_INTENT_RE`/`APPT_QUERY_RE` detection (task #109) only exists in the real `/inbound-sms` webhook route, not the simulator. Needs a real-phone retest to actually confirm or deny this as a real bug.
- **business_phone different-number branch** — during Brain 2 setup, answer that the business line is different from the personal cell, confirm it asks for and saves the separate number, and forwarding instructions reference the right one.
- **`forwardingTest.js`'s real outbound call** — dial the forwarding code, tell Brain 2 "done," confirm it places the real test call and correctly resolves conditional vs. unconditional forwarding.
- **GBP/Messenger self-serve walkthrough** — as a fresh contractor, follow the AI's copy-paste instructions for the GBP booking button and Messenger/Instagram auto-reply end to end, checking nothing assumes prior platform knowledge.
- **STOP mid-session** — reply STOP partway through an active Brain 3 conversation and confirm Twilio's native opt-out doesn't leave the session in a weird half-state.
